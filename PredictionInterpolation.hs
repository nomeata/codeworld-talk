...
module PredictionInterpolation
...
type EventQueue s = M.MultiMap (Timestamp, Timestamp, PlayerId) (Event s)
...
initFuture :: Double -> s -> Int -> Future s
initFuture smooth s numPlayers = Future
    { committed   = (0, s)
    , lastQuery   = 0
...
handleNextEventsSmooth :: StepFun s -> AnimationRate -> Double -> Timestamp ->
    EventQueue s -> TState s -> TState s
handleNextEventsSmooth step rate smooth now eq ts
    = foldl' (flip (handleNextEvent step rate)) ts $
      map (\((a,t,_p),h) -> (interpol smooth now a t,h)) $
      M.toList eq

interpol :: Double -> Timestamp -> Timestamp -> Timestamp -> Timestamp
interpol s now a t
    | x > 0 && x < 1 = x * t + (1 - x) * now
    | otherwise      = t
   where x = (now - a) / s
...
currentTimePasses = advanceFuture
...
addEvent :: StepFun s -> AnimationRate ->
    PlayerId -> Timestamp -> Timestamp -> Maybe (Event s) ->
    Future s -> Future s
  -- A future event.
{-
...
addEvent step rate player now real mbEvent f
    = recordActivity step rate player real $
      f { pending    = maybe id (M.insertR (now, real, player)) mbEvent $ pending f }
...
advanceCommitted step rate f
  ...
  where
    commitTime' = (minimum $ IM.elems $ lastEvents f) - smooth f
    canCommit (_,t,_) = t < commitTime'
    (eventsToCommit, uncommited') = M.spanAntitone canCommit (pending f)
...
currentState step rate now f = snd current'
  where
    current' =
        timePassesBigStep step rate now $
        handleNextEventsSmooth step rate (smooth f) now (pending f) $
        committed f
...
advanceFuture step rate target f
    ...
    | otherwise =
        advanceCommitted step rate $
        f { pending = pending', future = future' }
  where
    hasHappened (_,t,_) = t <= target
    (toPerform, future') = M.spanAntitone hasHappened (future f)
